 ...
 <?php
    ...
    if (isset($_POST['transaction_cat'])) {
        ...
        if ($transaction_cat == 'all' and  $payment_mtd == 'all' and $to == '' and $from != '') {
            ...
            $condition = "&& department_income.department_income_cat_id = $transaction_cat && transaction_date = '$from'";
        } else if ($transaction_cat == 'all' and  $payment_mtd == 'all' and $from != '' and $to != '') {
            $condition = "&& transaction_date BETWEEN '$from' and '$to'";
        } else if (($transaction_cat != 'all' and $payment_mtd == 'all' and $from != '' and $to != '')) {
            $condition = "&& department_income.department_income_cat_id = $transaction_cat && transaction_date BETWEEN '$from' and '$to'";
        } else if ($transaction_cat == 'all' and  $payment_mtd != 'all' and $from != '' and $to != '') {
            $condition = "&& payment_method_id = $payment_mtd && transaction_date BETWEEN '$from' and '$to'";
        } else {
            $condition = "&& department_income.department_income_cat_id = $transaction_cat && payment_method_id = $payment_mtd && transaction_date BETWEEN '$from' and '$to'";
        }
